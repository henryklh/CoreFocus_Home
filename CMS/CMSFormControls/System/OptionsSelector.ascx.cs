...
public partial class CMSFormControls_System_OptionsSelector : FormEngineUserControl
{
    ...
    private bool IsUserAuthorizedToEditBizFormSQL
    {
        get
        {
            return MembershipContext.AuthenticatedUser.IsAuthorizedPerResource("CMS.Form", "EditSQLQueries");
        }
    }


    private bool IsBizFormMode
    {
        get
        {
            return (Form != null) && (Form.FormType == FormTypeEnum.BizForm);
        }
    }
    ...
    public override object Value
    {
        get
        {
            ...
            // Fallback to original value if someone tried injectd SQL without permissions to edit in BizForm mode
            if (IsBizFormMode && (lstOptions.SelectedIndex == SqlSourceIndex) && (originalMode == ListSourceIndex) && !IsUserAuthorizedToEditBizFormSQL)
            {
    ...
    protected override void OnPreRender(EventArgs e)
    {
        ...
        {
            // Check permissions
            if (!IsUserAuthorizedToEditBizFormSQL && (SqlSourceIndex > 0))
            {
                if (lstOptions.SelectedIndex == SqlSourceIndex)
                {
                    // SQL query is selected - user can not modify the value
                    lstOptions.Enabled = false;
                    txtValue.Enabled = false;
                }
                else
                {
                    // Other option is selected - user can modify the value, but can not select SQL query
    ...
    public override object[,] GetOtherValues()
    {
        ...
        if (lstOptions.SelectedIndex == SqlSourceIndex)
        {
            // Keep original value if user is not authorized to modify SQL query
            if (IsBizFormMode && !IsUserAuthorizedToEditBizFormSQL)
            {
                if (originalMode == SqlSourceIndex)
                {
                    values[0, 1] = Form.Data.GetValue(QueryColumnName);
                    values[1, 1] = null;
                }
                else if (originalMode == MacroSourceIndex)
                {
                    txtValue.Editor.ValueIsMacro = true;

                    values[0, 1] = null;
                    values[1, 1] = Form.Data.GetValue(MacroColumnName);
                }
            }
            else
            {
