...
public partial class DEVModules_Ecommerce_Controls_ProductOptions_ShoppingCartItemSelector : CMSUserControl
{
    #region "Variables"
    ...
    public bool SingleCartItemOnly
    {
        get;
        set;
    }
    ...
    private void AddProductToShoppingCart()
    {
        ...
        // Keio added - 201608171025
        // remove all item before adding new for this project only
        // if (SingleCartItemOnly)
        // ShoppingCartInfoProvider.EmptyShoppingCart(ShoppingCart);
        //Henry modified function * 20170622

        DepartmentInfo SessoinDepartment = DepartmentInfoProvider.GetDepartmentInfo("Session", SiteContext.CurrentSiteName);
        DepartmentInfo SessoinUnlimitedDepartment = DepartmentInfoProvider.GetDepartmentInfo("SessionUnlimited", SiteContext.CurrentSiteName);
        ...
        if (SingleCartItemOnly)
        {
            ...
            try
            {
                ...
                foreach (ShoppingCartItemInfo cartItem in cart.CartItems)
                {
                    ...
                    if (cartItem.SKU.SKUDepartmentID == SessoinDepartment.DepartmentID || cartItem.SKU.SKUDepartmentID == SessoinUnlimitedDepartment.DepartmentID)
                    {
                        if (cartItem != null)
                        {
    ...
    {
        ltlTest.Text = "at there: " + ShoppingCart.ShoppingCartID.ToString();
        foreach (ShoppingCartItemInfo s in ShoppingCart.CartItems)
        {
            ShoppingCart.RemoveShoppingCartItem(s.CartItemGUID);
        }
    }
}