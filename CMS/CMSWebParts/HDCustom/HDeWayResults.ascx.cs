...
public partial class CMSWebParts_HDCustom_HDeWayResults : CMSAbstractWebPart
{
    #region "Variables"
    ...
    protected void SetupControl()
    {
        ...
        else
        {
            ...
            {
                // live soap settings
                soapUrl = SettingsKeyInfoProvider.GetStringValue(SiteContext.CurrentSiteName + ".HDeWayRapidAPISoap");
                apiKey = SettingsKeyInfoProvider.GetStringValue(SiteContext.CurrentSiteName + ".HDeWayRapidAPIKey");
                apiPassword = SettingsKeyInfoProvider.GetStringValue(SiteContext.CurrentSiteName + ".HDeWayRapidAPIPassword");
            }
            else
            {
                // sandbox soap settings
                soapUrl = SettingsKeyInfoProvider.GetStringValue(SiteContext.CurrentSiteName + ".HDeWaySandboxSoap");
                apiKey = SettingsKeyInfoProvider.GetStringValue(SiteContext.CurrentSiteName + ".HDeWaySandboxKey");
                apiPassword = SettingsKeyInfoProvider.GetStringValue(SiteContext.CurrentSiteName + ".HDeWaySandboxPassword");
            }
            ...
            using (Rapid30 service = new Rapid30())
            {
                ...
                if (eWayProvider.IsPaymentCompleted)
                {
                    ...
                    string redirectUrl = SettingsKeyInfoProvider.GetStringValue(SiteContext.CurrentSiteName + ".HDeWaySuccessfulUrl");
                ...
                else
                {
                    ...
                    string HDeWayFailedUrl = SettingsKeyInfoProvider.GetStringValue(SiteContext.CurrentSiteName + ".HDeWayFailedUrl");

                    string HDeWayCancelledUrl = SettingsKeyInfoProvider.GetStringValue(SiteContext.CurrentSiteName + ".HDeWayCancelledUrl");
    ...
    private bool IsLivePayment
    {
        get
        {
            return ValidationHelper.GetBoolean(SettingsKeyInfoProvider.GetBoolValue(SiteContext.CurrentSiteName + ".HDeWayIsLive"), false);
        }
    }
