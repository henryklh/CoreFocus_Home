...
public partial class CMSWebParts_DEVCustom_Ecommerce_zdFitnessBooking : CMSAbstractWebPart
{
    ...
    protected bool checkBookingAvailable(int classItemID)
    {
        ...
        List<CustomTableItem> c = CustomTableItemProvider.GetItems(zdFitnessbookingCredit).WhereEquals("UserId", ValidationHelper.GetInteger(CurrentUser.UserID, 0)).WhereContains("ClassType", ValidationHelper.GetString(t[0].GetValue("ClassType"), "")).WhereGreaterThan("DateTimeEnd", DateTime.Now).OrderByDescending("ItemCreatedWhen").ToList();
        ...
        if (chkAgree.Checked)
        {
            ...
            if (ValidationHelper.GetInteger(t.GetValue("Capacity"), 0) > bookingClassPeople.Count)
            {
                ...
                if (b == null)
                {
                    ...
                    CustomTableItem c = CustomTableItemProvider.GetItems(zdFitnessbookingCredit).WhereEquals("UserID", CurrentUser.UserID).WhereGreaterThan("DateTimeEnd", DateTime.Now.AddDays(1)).WhereContains("ClassType", ValidationHelper.GetString(result[2].ToString(), "")).OrderByDescending("DateTimeEnd").FirstObject;
