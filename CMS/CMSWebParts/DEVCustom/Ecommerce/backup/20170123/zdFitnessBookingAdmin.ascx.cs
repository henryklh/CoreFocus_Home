            ...
            if (checkBookingAvailable(ValidationHelper.GetInteger(ClassID.Value, 0), u))
            {
                ...
                if ((ValidationHelper.GetInteger(t.GetValue("Capacity"), 0) > bookingClassPeople.Count))
                {
                    ...
                    if (b == null)
                    {
                        ...
                        CustomTableItem c = CustomTableItemProvider.GetItems(zdFitnessbookingCredit).WhereEquals("UserID", u.UserID).WhereGreaterThan("DateTimeEnd", DateTime.Now.AddDays(-1)).WhereContains("ClassType", ValidationHelper.GetString(ClassType, "")).OrderByDescending("DateTimeEnd").FirstObject;
                        if (c != null)
                        {
