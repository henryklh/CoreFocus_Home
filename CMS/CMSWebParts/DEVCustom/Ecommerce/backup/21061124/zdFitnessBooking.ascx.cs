...
public partial class CMSWebParts_DEVCustom_Ecommerce_zdFitnessBooking : CMSAbstractWebPart
{
    ...
    protected bool checkBookingAvailable(int classItemID)
    {
        ...
        List<CustomTableItem> c = CustomTableItemProvider.GetItems(zdFitnessbookingCredit).WhereEquals("UserId", ValidationHelper.GetInteger(CurrentUser.UserID, 0)).WhereContains("ClassType", ValidationHelper.GetString(t[0].GetValue("ClassType"), "")).WhereGreaterThan("DateTimeEnd", DateTime.Now).OrderByAscending("ItemCreatedWhen").ToList();
        ...
        if (c.Count() > 0)
        {
            ...
            if (d < classStartDateTime.AddHours(-6))
            {
                return false;
            }
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        if (chkAgree.Checked)
        {
            ...
            List<CustomTableItem> bookingClassPeople = CustomTableItemProvider.GetItems(zdFitnessbooking).WhereEquals("ClassID", ValidationHelper.GetInteger(result[1].ToString(), 0)).ToList();
    ...
    protected void btnCancel_Click(object sender, EventArgs e)
    {
        if (chkCancel.Checked)
        {
            ...
            DataClassInfo Fitnessbooking = DataClassInfoProvider.GetDataClassInfo(zdFitnessbooking);
            CustomTableItem b = CustomTableItemProvider.GetItems(zdFitnessbooking).WhereEquals("ClassID", ValidationHelper.GetInteger(btnCancel.CommandArgument, 0)).WhereEquals("UserID", CurrentUser.UserID).FirstObject;
            if (b != null)
            {
