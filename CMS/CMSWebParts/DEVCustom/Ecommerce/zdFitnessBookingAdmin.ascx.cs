...
public partial class CMSWebParts_DEVCustom_Ecommerce_zdFitnessBookingAdmin : CMSAbstractWebPart
{
    ...
    private void GenerateTimeTable(DateTime d)
    {
        ...
        {
            d = d.AddDays(-7);
            List<CustomTableItem> n = CustomTableItemProvider.GetItems(zdFitnessTimeTable).WhereGreaterThan("StartDateTime", d.Date).WhereLessThan("StartDateTime", d.Date.AddDays(1)).WhereNotEquals("Capacity", 0).OrderByAscending("StartDateTime").ToList();
