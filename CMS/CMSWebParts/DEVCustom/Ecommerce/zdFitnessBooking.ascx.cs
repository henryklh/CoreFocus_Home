...
public partial class CMSWebParts_DEVCustom_Ecommerce_zdFitnessBooking : CMSAbstractWebPart
{
    ...
    public void LoadBookingDates()
    {
        ddlDate.Items.Clear();
        string preselect = QueryHelper.GetString("date", "");

        DateTime x = DateTime.Now;
        for (int h = 0; h < 30; h++)
        {
            DateTime y = x.AddDays(h);
            string currentdate = y.ToString("dd/MM/yyyy") + " - " + y.DayOfWeek;
            string date = "date=" + y.ToString("dd-MM-yyyy");
            ddlDate.Items.Add(new ListItem(currentdate, date));
            ddlDate2.Items.Add(new ListItem(currentdate, date));
        }
        if (!string.IsNullOrEmpty(preselect))
        {
            preselect = "date=" + preselect;
            ddlDate.Items.FindByValue(preselect).Selected = true;
            ddlDate2.Items.FindByValue(preselect).Selected = true;
        }
    }

    protected void ddlDate_SelectedIndexChanged(object sender, EventArgs e)
    {
        Response.Redirect(CurrentDocument.AbsoluteURL + "?" + ddlDate.SelectedValue);
    }

    protected void ddlDate2_SelectedIndexChanged(object sender, EventArgs e)
    {
        Response.Redirect(CurrentDocument.AbsoluteURL + "?" + ddlDate2.SelectedValue);
    }
    ...
    public void LoadCustomerPurchase()
    {
        ...
        if (customer != null)
        {
            ...
            List<OrderInfo> orders = OrderInfoProvider.GetOrders().WhereEquals("OrderCustomerID", customer.CustomerID).WhereEquals("OrderStatusID", 3).OrderByDescending("OrderID").Take(7).ToList();
            foreach (OrderInfo m in orders)
            {
                //OrderItemInfo item1 = OrderItemInfoProvider.GetOrderItems(m.OrderID).OrderByDescending("OrderItemLastModified");
                foreach (OrderItemInfo f in OrderItemInfoProvider.GetOrderItems(m.OrderID))
                {
                    int skuid = f.OrderItemSKUID;
                    // Gets a TreeProvider instance
                    TreeProvider tree = new TreeProvider(MembershipContext.AuthenticatedUser);

                    // Gets the product page
                    var node = DocumentHelper.GetDocuments("CMS.Product")
                        .OnCurrentSite()
                    ...
                    if (node != null)
                    {
                        string p = CMS.Helpers.ValidationHelper.GetString(node.GetValue("ProductClassType").ToString(), "");
                        if (!string.IsNullOrEmpty(p))
                        {
                            if (p == "Class/Traing Session")
                            {
                                orderinfos.Add(m);
                            }
                        }
            ...
            if (orders != null)
            {
                lab1.Text += "," + orderinfos.Count().ToString();
                pnlYourPurchase.Visible = true;
                rptYourPurchase.DataSource = orderinfos;
    ...
    protected void rptYourPurchase_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
        {
            ...
            CMS.DocumentEngine.TreeNode productnode = new CMS.DocumentEngine.TreeNode();
            //OrderItemInfo item1 = OrderItemInfoProvider.GetOrderItems(order.OrderID).OrderByDescending("OrderItemLastModified");
            foreach (OrderItemInfo f in OrderItemInfoProvider.GetOrderItems(order.OrderID))
            {
                int skuid = f.OrderItemSKUID;
                // Gets a TreeProvider instance
                TreeProvider tree = new TreeProvider(MembershipContext.AuthenticatedUser);
            ...
            if (customTable != null)
            {
                ...
                if (CreditItem == null)
                {
                    ...
                    if (ValidationHelper.GetString(item1.GetValue("OrderItemSKUName"), "").ToLower().Contains("unlimited"))
                    {
                        ...
                        // 5 months
                        if (ValidationHelper.GetString(item1.GetValue("OrderItemSKUName"), "").ToLower().Contains("5 months"))
                        {
                            expiryDate = getExpiryDate(ValidationHelper.GetDateTime(item1.GetValue("OrderItemLastModified"), DateTime.Now, ""), 150);
                            credit = 150;
                        }
                    ...
                    if (ValidationHelper.GetString(item1.GetValue("OrderItemSKUName"), "").ToLower().Contains("casual"))
                    {
                        ...
                        if (ValidationHelper.GetString(item1.GetValue("OrderItemSKUName"), "").ToLower().Contains("5 months"))
                        {
                            expiryDate = ValidationHelper.GetDateTime(item1.GetValue("OrderItemLastModified"), DateTime.Now, "").AddDays(150);
                        }
                        newCustomTableItem.SetValue("DateTimeEnd", expiryDate);
                        newCustomTableItem.Insert();
                    }
                    ...
                    if (credit == 0)
                    {
                        credit = GetOrderCredits(ref item1, ref order, ref expiryDate, ref message, ref newCustomTableItem, productnode);
                        //newCustomTableItem.Insert();
                    }
            ...
            if (Label1.Text.ToLower().Contains("classes"))
            {
                Label3.Text = Label1.Text.Replace(" classes", "");
            }
    ...
    public int GetOrderCredits(ref OrderItemInfo orderitem, ref OrderInfo order, ref DateTime expiryDate, ref string message, ref CustomTableItem newCustomTableItem, CMS.DocumentEngine.TreeNode productnode = null)
    {
        int credits = 0;
        credits = GetCreditsFromProducts(ref orderitem, ref order, ref credits, ref expiryDate, ref message, ref newCustomTableItem, productnode);
        return credits;
    }

    public int GetCreditsFromProducts(ref OrderItemInfo orderitem, ref OrderInfo order, ref int lineCredits, ref DateTime expiryDate, ref string message, ref CustomTableItem newCustomTableItem, CMS.DocumentEngine.TreeNode productnode = null)
    {
        if (productnode != null)
        {
            string ProductClassLimits = productnode.GetStringValue("Class", "");
            int SessionLength = productnode.GetIntegerValue("SessionLength", 0);
            switch (ProductClassLimits)
            {
                case "Unlimited":

                    expiryDate = order.OrderDate.AddDays(SessionLength);
                    newCustomTableItem.SetValue("DateTimeEnd", expiryDate);
                    newCustomTableItem.SetValue("ClassType", "TRX|REFORMER");
                    newCustomTableItem.SetValue("IsUnlimited", 1);
                    double c = (SessionLength * 0.86);
                    c = Math.Round(c, 0, MidpointRounding.AwayFromZero);
                    lineCredits = ValidationHelper.GetInteger(c, 0);
                    newCustomTableItem.SetValue("PeriodMax", lineCredits);
                    newCustomTableItem.Insert();
                    return lineCredits;

                default:
                    int classlimits = ValidationHelper.GetInteger(ProductClassLimits, 0);
                    if (classlimits > 0)
                    {
                        expiryDate = order.OrderDate.AddDays(SessionLength);
                        newCustomTableItem.SetValue("DateTimeEnd", expiryDate);
                        newCustomTableItem.SetValue("ClassType", "TRX|REFORMER");
                        newCustomTableItem.SetValue("IsUnlimited", 0);
                        lineCredits = classlimits;
                        newCustomTableItem.SetValue("RemainCounter", lineCredits);
                        newCustomTableItem.Insert();
                        return lineCredits;
                    }
                    break;
