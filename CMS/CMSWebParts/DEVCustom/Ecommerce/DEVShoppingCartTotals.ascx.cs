...
public partial class CMSWebParts_DEVCustom_DEVShoppingCartTotals : CMSCheckoutWebPart
{
    #region "Constructor"
    ...
    /// </summary>
    public CMSWebParts_DEVCustom_DEVShoppingCartTotals()
    {
    ...
    public void SetupControl()
    {
        ...
        switch (type)
        {
            ...
            case "TotalDiscount":
                ...
                // updated by Keio to apply the total discount include tax on product SKU.
                //value = ShoppingCart.OrderDiscount + ShoppingCart.ItemsDiscount + ShoppingCart.ShippingDiscount;

                var ItemsTotalPriceIncludingOptions = ShoppingCart.CartContentItems.Where(x => x.TotalPriceIncludingOptions > 0);

                double TotalItemsDiscount = 0;
                //ltlTest.Text += "<br/>ItemsTotalPriceIncludingOptions: " + ItemsTotalPriceIncludingOptions.Count().ToString();

                foreach(ShoppingCartItemInfo scii in ItemsTotalPriceIncludingOptions){

                    double iPrice = scii.SKU.GetDoubleValue("SKUPriceIncTax", 0);

                    if(iPrice == 0)
                    {
                        iPrice = scii.SKU.SKUPrice;
                    }

                    //ltlTest.Text += "<br/>" + scii.SKU.SKUName + ": " + iPrice + " - " + scii.UnitTotalPrice + " = " + (iPrice - scii.UnitTotalPrice);
                    TotalItemsDiscount += (iPrice - scii.UnitTotalPrice) * scii.CartItemUnits;
                }

                value = ShoppingCart.OrderDiscount + TotalItemsDiscount + ShoppingCart.ShippingDiscount;
