...
public partial class CMSWebParts_DEVCustom_Membership_Profile_DEVMyAccount : CMSAbstractWebPart
{
    ...
    /// <summary>
    /// Indicates whether use bootstrap navbar.
    /// </summary>
    public bool UseBootstrapNavbar
    {
        get
        {
            return ValidationHelper.GetBoolean(GetValue("UseBootstrapNavbar"), false);
        }
        set
        {
            SetValue("UseBootstrapNavbar", value);
        }
    }
    ...
    protected void SetupControl()
    {
        ...
        else
        {
            ...
            if (AuthenticationHelper.IsAuthenticated())
            {
                ...
                if (UseBootstrapNavbar)
                {


                    ltlNavTitle.Text = page.Substring(0, 1).ToUpper() + page.Substring(1);
                   
                    ltlNavLinks.Text = string.Empty;
                    try
                    {
                        for (int i = 0; i < tabMenu.TabItems.Count; i++)
                        {
                            var x = tabMenu.TabItems[i];
                            if (i == tabMenu.SelectedTab)
                            {
                                ltlNavLinks.Text += "<li class=\"nav-item\"><a class=\"active nav-link " + x.Text.Replace(" ", "") + "\" target=\"_self\" href=\"" + x.RedirectUrl + "\" >" + x.Text + "</a></li>";
                            }
                            else
                            {
                                ltlNavLinks.Text += "<li class=\"nav-item\"><a class=\"nav-link " + x.Text.Replace(" ", "") + "\" target=\"_self\" href=\"" + x.RedirectUrl + "\"  >" + x.Text + "</a></li>";
                            }

                        }

                        ltlNavTabs.Text = ltlNavLinks.Text;
                    }
                    catch (Exception ex)
                    {
