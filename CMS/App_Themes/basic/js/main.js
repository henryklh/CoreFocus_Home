...
(function ($) {
	...
	var Boss = {
		...
		isotopeFilter: function () {
			...
			$(self.container).find('a[data-filter]').on('click', function (e) {
			    var $this = $(this),
                        selector = $this.attr('data-filter');
			    filterContainer.find('.active').removeClass('active');
			    ...
			    filterContainer.find('a[data-filter="' + selector + '"]').addClass('active');
	...
	$(window).on('load', function() {
		...
		if ($.fn.mmenu) {
		    if ($('#MobileNav').length > 0) {
		        ...
		        // - close button for user friendly (some people may not know click on page area will close the panel.)

		        $(".mm-navbar").append("<button class='mm-close-button'><i class='fa fa-times-circle'></i></button>")
